...
# creation of database if file not already created (handles both users and phrases tables)
def init_db():
...
def verify(req: VerifyRequest):
    ...
    # SCENARIO G_Phase2: semantic match
        ...
        # semantic check
        similarity_score = calculate_similarity(req.input_text, stored_embeddings)
        logger.info(f"User: {req.user_id} | Input: {req.input_text} | Score: {similarity_score:.2f}")
